...
To calculate wt and wp there are two options:
1) if `autoWeights` is `Y`, a csv file is read, which contains the data for a series of sites of which there are available records of the production of seed. This data frame includes the id of the sites, the coordinates and the best wp and wt obtained comparing the observation with the forecasted probabilities by using the function mastFaSyl. Thus, a calculation of the minimum distance among the interestd point and all those of the data frame is performed and wt and wp are taken from the nearest one;
2) if `autoWeights` is `N`, wt and wp takes the values of 3 and 1 respectively, which represents the best average combination for predicting seed production when comparing the forecasts with the field data (wt = 3 and wp = 1).
